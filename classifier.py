...
# # Load small open-source model (runs locally)
...
#     """
#     Classifies the ticket as AI_PATCH or VIBE_WORKFLOW
#     with reasoning and checklist generation.
#     """
#     channel = ticket.get("channel", "").lower()
#     severity = ticket.get("severity", "").lower()
#     summary = ticket.get("summary", "").lower()

#     # More complex heuristic-based logic
#     ai_keywords = [
#         "crash", "exception", "stack trace", "bug", "api failure", "timeout", "error", "patch", "nullpointer"
#     ]
#     workflow_keywords = [
#         "config", "access", "permission", "dashboard", "login", "ui", "connectivity", "workflow"
#     ]

#     ai_score = sum(kw in summary for kw in ai_keywords)
#     wf_score = sum(kw in summary for kw in workflow_keywords)
...
#     # Channel influence (chat/email/web)
#     if channel == "chat":
#         wf_score += 1
#     elif channel == "email":
#         ai_score += 1

#     decision = "AI_PATCH" if ai_score >= wf_score else "VIBE_WORKFLOW"

#     reasoning = generate_reasoning(ticket, decision)
#     checklist = generate_checklist(ticket, decision)

#     return {"decision": decision, "reasoning": reasoning, "checklist": checklist}


# def generate_reasoning(ticket, decision):
#     """Generate dynamic reasoning text using local model."""
#     prompt = (
#         f"You are an AI assistant classifying banking support tickets.\n"
#         f"Ticket: {ticket}\nDecision: {decision}\n"
#         f"Explain briefly why this decision makes sense."
#     )
...
# def generate_checklist(ticket, decision):
#     """Generate 3–5 next-step checklist items dynamically."""
#     base_steps_ai = [
#         "Reproduce the issue in staging.",
#         "Collect relevant logs and stack traces.",
#         "Run unit tests for impacted modules.",
#         "Prepare AI patch using the model suggestions.",
#         "Deploy fix and monitor live metrics.",
#     ]
#     base_steps_wf = [
#         "Confirm configuration in the admin console.",
#         "Validate user permissions and roles.",
#         "Sync latest workflow scripts.",
#         "Test dashboard access end-to-end.",
#         "Report status to customer support.",
#     ]
#     steps = base_steps_ai if decision == "AI_PATCH" else base_steps_wf
#     return random.sample(steps, k=5)


# def clean_generated_text(text, prompt):
#     """Strip the prompt part and cut off extra junk."""
#     if prompt in text:
